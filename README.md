...
The repository includes Beowulf SDKs for runtime decisioning:

- Python SDK: `sdks/python/beowulf`
- .NET SDK: `sdks/dotnet/beowulf`
- Node.js SDK: `sdks/node/beowulf`

Install .NET SDK from NuGet:

```bash
dotnet add package Beowulf.Cedar
```

Install Node.js SDK from npm:

```bash
npm install beowulf-cedar-sdk
```

Chocolatey is intended for executable tools. Keep SDK/library distribution on NuGet, and use Chocolatey for an optional CLI wrapper.
